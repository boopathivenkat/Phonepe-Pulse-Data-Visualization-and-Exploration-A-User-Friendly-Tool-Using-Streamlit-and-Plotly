...
columns = ["State", "Years", "Quarter","Pincodes", "Registered Users"]
...
def topdisttr(df,state):
    ...
    agusstg = agusst.groupby("Districts")[["Transaction Count","Transaction Amount"]].sum()
    agusstg = agusstg.reset_index()
    
    colors = px.colors.qualitative.Plotly[:len(agusst)]

    fig_bar = px.bar(data_frame=agusstg, x="Districts", y="Transaction Count", hover_name="Districts",
                    width=980, height=600,text_auto='.3s', title=f"{state} Quarter Districts Wise Transaction Count",
                    color="Districts", color_discrete_sequence=colors)

    fig_bar1 = px.bar(data_frame=agusstg, x="Districts", y="Transaction Amount", hover_name="Districts",
                width=980, height=600,text_auto='.3s', title=f"{state} Districts Wise Transaction Amount",
                color="Districts", color_discrete_sequence=colors)
...
def topdistus(df, state):
    ...
    agusstg = agusst.groupby("Districts")[["Registered Users", "App Opens"]].sum()
    agusstg = agusstg.reset_index()
    
    fig = px.sunburst(agusstg, values="App Opens", path=["Districts","Registered Users","App Opens"],
...
def top_tran_amount_year(option, year):
    ...
    toptryqg = toptry.groupby("State")[["Transaction Count", "Transaction Amount"]].sum()
    ...
    with coll3:
        fig_amount = px.bar(toptryqg, x="State", y="Transaction Amount", title=f"{year} Transaction Amount",
                                color="Transaction Amount", color_continuous_scale="ylgnbu",
                                range_color=(toptryqg["Transaction Amount"].min(), toptryqg["Transaction Amount"].max()), height=650, width=600)
        #st.plotly_chart(fig_amount)

        fig_ind_1 = px.choropleth(toptryqg, geojson=data1, locations="State", featureidkey="properties.ST_NM",
                                    color="Transaction Amount", color_continuous_scale="ylgnbu",
                                    range_color=(toptryqg["Transaction Amount"].min(), toptryqg["Transaction Amount"].max()), hover_name="State",
                                    title=f"{year}", fitbounds="locations", height=650, width=600)
        fig_ind_1.update_geos(visible=False)
        #st.plotly_chart(fig_ind_1)

    with coll4:
        fig_count = px.bar(toptryqg, x="State", y="Transaction Count", title=f"{year} Transaction Count",
                            color="Transaction Amount", color_continuous_scale="tempo",
                            range_color=(toptryqg["Transaction Amount"].min(), toptryqg["Transaction Amount"].max()), height=650, width=600)
        #st.plotly_chart(fig_count)

        fig_ind_2 = px.choropleth(toptryqg, geojson=data1, locations="State", featureidkey="properties.ST_NM",
                                    color="Transaction Count", color_continuous_scale="tempo",
                                    range_color=(toptryqg["Transaction Count"].min(), toptryqg["Transaction Count"].max()), hover_name="State",
                                    title=f"{year}", fitbounds="locations", height=650, width=600)
        fig_ind_2.update_geos(visible=False)
        #st.plotly_chart(fig_ind_2)
    return toptry
...
def top_tran_amount_year_quarter(option1, quarter):
    ...
    toptryqg = toptryq.groupby("State")[["Transaction Count", "Transaction Amount"]].sum()
    toptryqg.reset_index(inplace=True)
    ...
    with coll3:
        fig_amount = px.bar(toptryqg, x="State", y="Transaction Amount", title=f"{quarter}-Quarter Transaction Amount",
                                color="Transaction Amount", color_continuous_scale="ylgnbu",
                                range_color=(toptryqg["Transaction Amount"].min(), toptryqg["Transaction Amount"].max()),height=600, width=700)
        st.plotly_chart(fig_amount)
    ...
    with coll4:
        ...
        fig_count = px.bar(toptryqg, x="State", y="Transaction Count", title=f"{quarter}-Quarter Transaction Count",
                            color="Transaction Count", color_continuous_scale="tempo",
                            range_color=(toptryqg["Transaction Amount"].min(), toptryqg["Transaction Amount"].max()), height=600, width=700)
...
def toptrpins(df, state):
    ttyqu = df[df["State"]==state]
    ttyqu.reset_index(drop=True, inplace=True)

    coll3, coll4 = st.columns(2)
    with coll3:    
        fig = px.bar(ttyqu, y="Transaction Amount", x="Quarter",hover_data="Pincodes",color="Transaction Amount",
                    hover_name="Pincodes", width=650, height=600)
        st.plotly_chart(fig)

    with coll4:
        fig1 = px.bar(ttyqu, y="Transaction Count", x="Quarter",hover_data="Pincodes",color="Transaction Count", 
                    hover_name="Pincodes", width=650, height=600)
...
with st.sidebar:
    ...
    selected = option_menu("Main Menu", ["Intro",'Explore Data','Contact Us'], 
    ...
    **Services -** Digital payments Mobile payments Payment 
    ...
    with tab3:
        ...
        if tab_selected == "Top_transaction":
            ...
            state = st.selectbox('Select a State', Top_transaction["State"].unique(), key='state_selectbox_9')
            toptrpins(tty,state)
